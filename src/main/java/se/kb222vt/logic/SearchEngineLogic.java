...
public class SearchEngineLogic {
	...
	public List<Page> search(String query){
		...
		ArrayList<Page> result = wordFrequency(query);
		normalizedScore(result, true);
		Collections.sort(result, Page.getArticleByScore());
		return result.subList(0, 5);
	}
	
	/**
	 * Get articles with the word "query" in them and sets the score of the article to the number of times it appears
	 * @param query
	 * @return list of blogs with the word from param query in them
	 */
	//------------------------- TODO ---------------------------------
	//TODO: @param query should be a array of words from search query
	...
	//------------------------- ---- ---------------------------------
	public ArrayList<Page> wordFrequency(String query) {
		...
		int wordID = getIdForWord(query); //the user is searching for hashCode
		//lets see which article has this word
		for(Page p : Application.articles.values()) {
			int instanceOfWord = p.getInstancesOfWord(wordID);
			if(instanceOfWord > 0) {
				Page hitPage = new Page(p);
				hitPage.setScore(instanceOfWord);
				result.add(hitPage);
			}
		}
		return result;
	...
	public int getIdForWord(String word) {
		return Application.wordMap.get(word);
	}
	
	/**
	 * Normalize scores for pages
	 * Algorithm inspiration from: http://coursepress.lnu.se/kurs/web-intelligence/files/2018/10/4.-Search-Engines.pdf page 19
	 * @param score
	 * @param biggerBetter
	 */
	public void normalizedScore(ArrayList<Page> pages, boolean biggerBetter) {
		if(biggerBetter) {			
			double maxScore = 0;
